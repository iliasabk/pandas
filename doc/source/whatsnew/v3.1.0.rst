...
- Fixed segfault in :meth:`DataFrame.to_json` and :meth:`Series.to_json` with ``date_format="iso"`` when an allocation inside the ISO datetime, timedelta, or :class:`datetime.date` string callbacks failed; the pending error (e.g. ``MemoryError``) now propagates instead of dereferencing ``NULL`` (:issue:`69461`)
